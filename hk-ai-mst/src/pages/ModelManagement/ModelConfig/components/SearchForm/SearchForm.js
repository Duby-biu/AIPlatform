...
// 数据操作引入
import { searchTableData } from '../../../../../api/modelManagement/modelConfig';
    ...
    this.props.form.validateFields((err, values) => {
      ...
      searchTableData(values).then((response) => {
        ...
        response.data.dataSource.map((item) => {
          ...
          return item.key = item.historyId;
        });
              ...
              <AntDFormItem label='状态'>
                {getFieldDecorator('state')(
                  <AntDSelect allowClear={true}>
                    <AntDOption value='0'>已配置</AntDOption>
                    <AntDOption value='1'>训练中</AntDOption>
                    <AntDOption value='2'>中止训练</AntDOption>
                    <AntDOption value='3'>训练完成</AntDOption>
                    <AntDOption value='4'>验证中</AntDOption>
                    <AntDOption value='5'>验证完成</AntDOption>
                    <AntDOption value='-1'>异常</AntDOption>
                  </AntDSelect>
                )}
              </AntDFormItem>
            </AntDCol>
            <AntDCol span={8} style={{ textAlign: 'right' }}>
